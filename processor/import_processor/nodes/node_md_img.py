...
class NodeMDImg(BaseNode):
    ...
    def _clean_minio_directory(self, minio_client: Minio, prefix: str):
        """
        ...
        """
        try:
            list_object = minio_client.list_objects(minio_config.bucket_name, prefix=prefix, recursive=True)
            delete_list = [DeleteObject(obj) for obj in list_object]
            if delete_list:
    ...
    def _upload_to_minio(self, minio_client: Minio, local_path: str, object_name: str) -> str | None:
        ...
        try:
            ...
            base_url = f"{minio_config.endpoint}/{minio_config.bucket_name}"
